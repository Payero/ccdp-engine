...
{
  echo ''
  echo 'Runs the class used to remove Topics and Queues from an ActiveMQ '
  echo 'server. ${CCDP_HOME}/config/ccdp-config.json unless another one is '
  echo 'provided using the -c argument'
  echo ''
  echo 'usage: $0'
  echo '   -h,--help         Shows this message'
  echo '   -c, --config-file Path to the configuration file.'
  echo '   -u, --amq-url     The URL where the ActiveMQ server is listening.'
  echo '   -q, --queue-names A comma delimited list of quesus or ALL.'
  echo '   -t, --topic-names A comma delimited list of topics or ALL.'
  echo ''
  exit 0
}
...
APP_ARGS=$@
JAVA_OPTS=""
JAVA_APP="com.axios.ccdp.utils.AmqCleaner"
