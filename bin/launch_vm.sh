...
usage()
{
  ...
  echo '   -f, --filename <arg>     The JSON file with the configuration'
  echo ''
  echo ' Any additional argument is concatenated and passed to the script'
  echo ''
  exit 0
}
...
CMD="${JAVA_CMD} -cp ${_CLASSPATH} ${JAVA_OPTS} ${JAVA_APP} $_ARGS"
